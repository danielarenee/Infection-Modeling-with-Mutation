"""
This script generates a two-panel figure for the I-feedback SIRCmw_I model:
...
# ==============================================================================
#transition_boundary = 0.033539
transition_boundary = 0.084248
# ==============================================================================
...
# ------------------------------------------------------------------------------
# RIGHT PANEL: Diagonal Profile Plot (eps1 = eps2) with Taylor approximations
# ------------------------------------------------------------------------------
...
SI_0 = 0.001  # Scaling factor is 0.001 for I-feedback model
MU_val = 0.02
ALPHA_val = 365.0 / 3.0
BETA0_val = 600.0

# 1. Compute Taylor series approximations for small epsilon (from approximation_small_eps.py)
I0_low = 0.00114320801603584
I1_low = 1.19411827561e-06
I2_low = 1.24757263691e-09
I3_low = 1.30184608959e-12
...
# 2. Compute perturbation series approximations for large epsilon (from approximation_large_eps.py)
# I(eps) = I_inf + h1_large / eps + h2_large / eps**2 + h3_large / eps**3
h1_large = -516.130857143
h2_large = -265439.940636
h3_large = 37097084.3903

# Calculate analytical upper bound I_inf = 1 - R_0^-1
R0 = BETA0_val / (MU_val + ALPHA_val)
I_inf = 1.0 - 1.0 / R0

# Avoid division by zero and vertical line artifacts by masking values outside [1e-4, 1.0] and small epsilons
...
# The high-epsilon expansion is only valid for large epsilon, so we restrict it to list_eps1_fine >= 0.6
# and values within the physical range [1e-4, 1.0]
mask_high1 = (list_eps1_fine >= 0.6) & (approx_high1 > 1e-4) & (approx_high1 < 1.0)
mask_high2 = (list_eps1_fine >= 0.6) & (approx_high2 > 1e-4) & (approx_high2 < 1.0)
mask_high3 = (list_eps1_fine >= 0.6) & (approx_high3 > 1e-4) & (approx_high3 < 1.0)
...
for approx_plot in [approx_high1_plot, approx_high2_plot, approx_high3_plot]:
    ...
    if len(valid_indices) > 0:
        ...
        if first_valid_idx > 0 and list_eps1_fine[first_valid_idx - 1] >= 0.6:
