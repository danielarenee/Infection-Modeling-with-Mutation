...
ax2.set_title('zoom on y (from -25 to 15)', fontsize=12)
...
E0 = te_plot[i0]
eps_E0  = E0 / scale_factor
print(f"\nE0 = {E0:.4f}  →  eps = {eps_E0:.2f}   (Re(λ₂) max = {re_lam2[i0]:.6f})")

# Print the maximum real part of each eigenvalue and all eigenvalues across the sweep
print("\nMaximum real part of eigenvalues across the entire sweep:")
print(f"  All eigenvalues combined: {np.max(eigvals.real):.8f}")
for k in range(4):
    print(f"  λ{k+1}: {np.max(eigvals[:, k].real):.8f}")
