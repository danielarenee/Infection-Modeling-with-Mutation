#!/usr/bin/env python3
"""
Prevalence roots sweep (Transmission-driven variant)

Computes the endemic equilibrium and local stability of the transmission-driven SIRCm model
over a logarithmic parameter grid of (eps1, eps2) by solving the polynomial reduction in I.
Saves the results to 'prevalence_roots_results.npz' for Figure 02 plotting.
"""
...
# Add SIRCm_transmission directory to path
...
def run_one_root(relative_eps1, relative_eps2):
    ...
    for S, I, R, C in roots:
        ...
        if max_real < 0.0:
            stable_roots.append((I, 1.0))
        else:
            unstable_roots.append((I, 0.0))
...
def main():
    print("Running algebraic endemic equilibrium sweep for transmission model on log scale centered at 0...")
