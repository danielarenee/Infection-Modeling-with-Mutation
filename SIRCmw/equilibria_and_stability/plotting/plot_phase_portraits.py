...
# Reference scaling factor (S0_ref * I0_ref = 0.0002) from notebook/papers
SI_0 = 0.0002
...
# Cache file path
CACHE_FILE = SCRIPT_DIR.parent / "sircmw_phase_portraits_cache.npz"
...
def generate_plots(mode, solutions, manual_flows):
    ...
    for i, te in enumerate(TILDE_EPS_LIST):
        ...
        if mode == 'manual':
            ...
            dS_mesh = mu * (1.0 - S_mesh) - beta0 * S_mesh * I_mesh + (1.0 + eps * S_mesh * I_mesh) * gamma * C_mesh
        ...
        ax1.text(0.95, 0.95, f'$\\tilde{{\\epsilon}} = {te}$', transform=ax1.transAxes, 
...
def main():
    print(f"Exact scaling factor SI_0: {SI_0}")
