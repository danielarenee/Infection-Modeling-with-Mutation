...
for the I-feedback SIRCmw model, extending the tilde epsilon range up to 10.0.
"""
...
# Range extension to 10.0 and 10000.0
const tilde_eps_MAX_10 = 10.0
const β0_MAX_10000 = 10000.0

# Try every real root of the cubic, keeping whichever yields a valid (S,I,R,C) 
...
function find_endemic_equilibrium(tilde_eps, beta0, sigma)
    ...
    for I_star in endemic_I
        ...
        eq === nothing && continue
        S, I, R, C = eq
        res = abs((1 - sigma)*beta0*C*I + p.α*I - R*(p.μ + (1.0 + eps*I)*p.δ))
        if res < best_res
