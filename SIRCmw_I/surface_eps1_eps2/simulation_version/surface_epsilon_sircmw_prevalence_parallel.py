...
def sircmw(t, y, p):
    """
    Modified SIRCm: the immunity-erosion rates are driven by incidence (I-feedback formulation)
    """
    S, I, R, C = y
    b = beta_t(t, p['beta0'], p['eps'])
    eps1_I_p1 = p['eps1']*I+1; eps2_I_p1 = p['eps2']*I+1
    dS = mu*(1 - S) - b*S*I + eps2_I_p1*gamma*C
    dI = b*S*I + sigma*b*C*I - (mu + alpha)*I
    dR = (1 - sigma)*b*C*I + alpha*I - (mu + eps1_I_p1*delta)*R
    dC = eps1_I_p1*delta*R - b*C*I - (mu + eps2_I_p1*gamma)*C
...
def infection_eigvec(y, t, p):
    ...
    a_SI = -b*S + e2*gamma*C                            # column-I couplings
    a_RI = (1 - sigma)*b*C + alpha - e1*delta*R
    a_CI = e1*delta*R - b*C - e2*gamma*C
    vI = 1.0
...
def run_simulation(relative_eps1, relative_eps2):
    # Scaling factor: choose between I_0 (new formulation) and S_0 * I_0 (original formulation)
    # scale_factor = y0_ts[0] * y0_ts[1] # SI_0 scaling
    scale_factor = y0_ts[1] # I_0 scaling
